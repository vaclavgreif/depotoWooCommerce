...
class Depoto_Payments
{
	...
	public function __construct($depoto_api_payments_pairs)
	{
	...
	private function add_field($id, $value, $depoto_payments_pairs)
	{
		add_settings_field(
			$id,
			__($value, 'depoto'),
			function ($args) use ($depoto_payments_pairs) {
				$id = esc_attr($args['label_for']);

				echo "<select name='$id' id='$id' >";
				foreach ($depoto_payments_pairs as $depoto_id => $depoto_title) {
					echo "<option value='$depoto_id' " . (($depoto_id === $this->depoto_payments_options[$id]) ? 'selected' : '') . ">$depoto_title</option>";
				}
				echo '</select>';
			},
			'depoto',
